...
public class PlayerVisionConeMouse : MonoBehaviour {
    ...
    void CheckLineOfSight() {
        ...
        for (int i = -viewAngle /2; i <= viewAngle /2 ; i++) {
            Vector3 forwardVector = Camera.main.ScreenToWorldPoint(Input.mousePosition) - transform.position;     
            RaycastHit2D sightTest = Physics2D.Raycast(transform.position, RotateVector(forwardVector, i), viewDistance, layerMask);
            if (sightTest.collider != null) {
                ...
                vertices.Add((Vector3)(RotateVector(forwardVector, i) * viewDistance) + new Vector3(0,0,-2f));
